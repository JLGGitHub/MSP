...
unsigned short Inf_Table[10];
unsigned short Inf_Table_Auxiliar[10];
...
void Iniciar_ADC(void){
	...
	//ADC12CTL0 = ADC12ON+ADC12MSC+ADC12SHT0_4; 			// Turn on ADC12, extend sampling time
...
void Iniciar_Timer(void){
	...
	//TA0CCR0 = 26000;									// se carga para 10 Ms en caso de que el reloj este en 20Mhz
	TA0CCR0 = 34000;
...
void Metodo_Leer_ADC(){
	...
	Vfiltro[0].x0=  Auxiliar_ADC;
			...
			qA= _Q5mpyI16(_Q5(a0),Vfiltro[0].x0) + _Q5mpyI16(_Q5(a1),Vfiltro[0].x1) + _Q5mpyI16(_Q5(a2),Vfiltro[0].x2) - _Q5mpyI16(_Q5(b1),Vfiltro[0].y1) - _Q5mpyI16(_Q5(b2),Vfiltro[0].y2);
			qA=  _Q5div(qA,_Q5(b0));
			Vfiltro[m].y0=  _Q5toF(qA);
...
void Metodo_Enviar_Tabla(){
	P5OUT= BIT2;
for (m = 1; m < 10; m++) {
			 if((m==1)&&(Bit_Habilitar[1]==true)&&(cEnableCambio[1]==true)){
				// Preparando la Trama
				Auxiliar_Leer_Puerto= 			Inf_Table[1]& 0x80;
				Informacion_Control= 			Auxiliar_Leer_Puerto>>3;
				Informacion_P_Digital= 			Inf_Table[1] & 0x7F;
				Informacion_Control=			Informacion_Control^Leer_Puerto;
				// Envio de la trama
				Enviar_serial_text(Flag_Inicio);
				...
				Enviar_serial_text(Bits_Crc_Calculado);
				Enviar_serial_text(Flag_Fin);
			}else if((m>1)&&(Bit_Habilitar[m]==true)&&(cEnableCambio[m]==true)){
				// Preparando la Trama
				Informacion_Control= 			Inf_Table[m] & 0x380;
				Informacion_Control= 			Informacion_Control>>3;
				Informacion_P_ADC= 				Inf_Table[m]&0x7f;
				Informacion_Control=Informacion_Control ^ Leer_ADC;
				// Envio de la trama
				Enviar_serial_text(Flag_Inicio);
...
void Actualizar_Tabla(){
	...
	Inf_Table[1]= P2IN&0x7f;
	ADC12CTL0 |= ADC12ENC +  ADC12SC;
	while (!(ADC12IFG & BIT0));
	Inf_Table[2]= ADC12MEM0>>2;
	while (!(ADC12IFG & BIT1));
	Inf_Table[3]= ADC12MEM1>>2;
	while (!(ADC12IFG & BIT2));
	Inf_Table[4]= ADC12MEM2>>2;
	while (!(ADC12IFG & BIT3));
	Inf_Table[5]= ADC12MEM3>>2;
	while (!(ADC12IFG & BIT4));
	Inf_Table[6]= ADC12MEM4>>2;
	while (!(ADC12IFG & BIT5));
	Inf_Table[7]= ADC12MEM5>>2;
	while (!(ADC12IFG & BIT6));
	Inf_Table[8]= ADC12MEM6>>2;
	while (!(ADC12IFG & BIT7));
	Inf_Table[9]= ADC12MEM7>>2;
  ...
  for (m=2;m<10;m++)
	{
		...
		Vfiltro[m].x0=  Inf_Table[m];
				...
				Inf_Table[m]=Vfiltro[m].y0;
	}
  for (m=2;m<10;m++)
  	{
  		Vfiltro2[m].x22 = Vfiltro2[m].x11;
  		Vfiltro2[m].x11 = Vfiltro2[m].x00;
  		Vfiltro2[m].y22 = Vfiltro2[m].y11;
  		Vfiltro2[m].y11 = Vfiltro2[m].y00;
  		Vfiltro2[m].x00=  Inf_Table[m];
  				//Y_0=((X_0*b0)+(X_1*b1)+(X_2*b2)-(Y_1*a1)-(Y_2*a2))/a0; Ecuacion del filtro...
  				qA= _Q4mpyI16(_Q4(a0),Vfiltro2[m].x00) + _Q4mpyI16(_Q4(a1),Vfiltro2[m].x11) + _Q4mpyI16(_Q4(a2),Vfiltro2[m].x22) - _Q4mpyI16(_Q4(b1),Vfiltro2[m].y11) - _Q4mpyI16(_Q4(b2),Vfiltro2[m].y22);
  				qA= _Q4div(qA,_Q4(b0));
  				Vfiltro2[m].y00=_Q4toF(qA);
  				Vfiltro2[m].y00= Vfiltro2[m].y00&0x3FE;
  				Inf_Table[m]=Vfiltro2[m].y00;
  	}
	for (m = 1; m < 10; m++){
		Inf_Table_Auxiliar[m]= Inf_Table[m];
	}
	for (m = 1; m < 10; m++) {
		if(Inf_Table[m]==Inf_Table_Auxiliar[m]){
			cEnableCambio[m]=true;
		}else{
			cEnableCambio[m]=false;
		}
	}
